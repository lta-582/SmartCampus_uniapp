	...
	<view class="header-wrap" >
		<view class="left-text">
			...
			<view :class="weeky==(i)?'text-content-today':'text-content'" v-for="i in 7" :key="i">
					<text class="week-txt"  :style="{ 'color': dMonth === dateDetailsLi[i] ? '#FFF' : 'unset', 'background-color': dMonth === dateDetailsLi[i] ? '#4070FF' : 'unset' }">{{ week[i]}}</text>
					<text class="date-txt"  :style="{ 'color': dMonth === dateDetailsLi[i] ? '#FFF' : 'unset','background-color': dMonth === dateDetailsLi[i] ? '#4070FF' : 'unset' }">{{ dateLi[i]}}</text>
			</view>
		</view>
		<u-line></u-line>
	</view>
